...
(defun expand-part-form-at-compile-time (form)
  ...
  (case (first form)
    ...
    ;; Phase T2: Datum definition
    (:datum
     (expand-datum-form-at-compile-time (rest form)))

    ;; Phase T3: Geometric Tolerancing (GD&T)
    (:flatness
     (expand-gdt-form-at-compile-time :flatness (rest form)))
    (:straightness
     (expand-gdt-form-at-compile-time :straightness (rest form)))
    (:circularity
     (expand-gdt-form-at-compile-time :circularity (rest form)))
    (:cylindricity
     (expand-gdt-form-at-compile-time :cylindricity (rest form)))
    (:perpendicularity
     (expand-gdt-form-at-compile-time :perpendicularity (rest form)))
    (:parallelism
     (expand-gdt-form-at-compile-time :parallelism (rest form)))
    (:angularity
     (expand-gdt-form-at-compile-time :angularity (rest form)))
    (:position
     (expand-gdt-form-at-compile-time :position (rest form)))
    (:concentricity
     (expand-gdt-form-at-compile-time :concentricity (rest form)))
    (:symmetry
     (expand-gdt-form-at-compile-time :symmetry (rest form)))

    ;; Profile tolerances
    (:profile-surface
     (expand-gdt-form-at-compile-time :profile-surface (rest form)))
    (:profile-line
     (expand-gdt-form-at-compile-time :profile-line (rest form)))

    ;; Runout tolerances
    (:circular-runout
     (expand-gdt-form-at-compile-time :circular-runout (rest form)))
    (:total-runout
...
;;; ============================================================================
;;; Phase T2: Datum Definition (GD&T)
;;; ============================================================================

(defun expand-datum-form-at-compile-time (args)
  "Expand (:datum \"A\" :on-face :direction :-z :extreme :min [:mmc|:lmc|:rfs])
  ...
  Examples:
    ...
    (:datum \"B\" :on-face :direction :+x :extreme :max :mmc t)
    (:datum \"A\" :on-face :type :cylindrical)

  The datum is stored in shape metadata for later GD&T references and export."

  (let ((label (first args))
        (selector-spec '())
        (material-condition :rfs))

    ;; Parse selector and material condition modifiers
    (loop for arg in (rest args)
          do (cond
               ;; Material condition modifiers
               ((and (eq arg :mmc) (not (eq arg t)))
                (setf material-condition :mmc))
               ((eq arg :mmc) nil)  ; Skip the 't' after :mmc
               ((and (eq arg :lmc) (not (eq arg t)))
                (setf material-condition :lmc))
               ((eq arg :lmc) nil)  ; Skip the 't' after :lmc
               ((and (eq arg :rfs) (not (eq arg t)))
                (setf material-condition :rfs))
               ((eq arg :rfs) nil)  ; Skip the 't' after :rfs
    ...
    (unless label
      (error ":datum requires a label (e.g., \"A\", \"B\", \"C\")"))

    (unless selector-spec
      (error ":datum requires a selector specification"))
    ...
    `(let* ((current-shape (get-result))
       ...
       ;; Add datum to shape metadata and update the shape
       (setf (clad.core:shape-metadata core-shape)
             (clad.gdt:add-datum-to-metadata
              (clad.core:shape-metadata core-shape)
              ,label
              ',selector-spec
              :material-condition ,material-condition))

       ;; Return nil (datum definition doesn't change the shape)
       nil)))
...
(defun expand-gdt-form-at-compile-time (gdt-type args)
  "Expand geometric tolerance forms at compile time.

  Syntax patterns:
    Form tolerances:
      (:flatness selector-spec :tolerance value)
      (:straightness selector-spec :tolerance value)
      (:circularity selector-spec :tolerance value)
      (:cylindricity selector-spec :tolerance value)

    Orientation tolerances:
      (:perpendicularity selector-spec :tolerance value :datum-ref \"A\" [:mmc t])
      (:parallelism selector-spec :tolerance value :datum-ref \"A\" [:mmc t])
      (:angularity selector-spec :tolerance value :datum-ref \"A\" :angle degrees [:mmc t])

    Location tolerances:
      (:position selector-spec :tolerance value :datum-refs (\"A\" \"B\" \"C\") [:mmc t])
      (:concentricity selector-spec :tolerance value :datum-ref \"A\")
      (:symmetry selector-spec :tolerance value :datum-ref \"A\")

  The tolerance is stored in shape metadata and will be exported to technical drawings."
  ...
  (let ((selector-spec '())
    ...
    ;; Parse arguments using index-based iteration
    (loop while (< i (length args))
          ...
          do (cond
               ...
               ((eq arg :tolerance)
                (incf i)
                (setf tolerance-zone (nth i args))
                (incf i))
               ((eq arg :datum-ref)
                (incf i)
                (setf datum-ref (nth i args))
                (incf i))
               ((eq arg :datum-refs)
                (incf i)
                (setf datum-refs (nth i args))
                (incf i))
               ((eq arg :angle)
                (incf i)
                (setf angle (nth i args))
                (incf i))
               ((eq arg :bilateral)
                (incf i)
                (setf bilateral (nth i args))
                (incf i))
               ((eq arg :mmc)
                (setf material-condition :mmc)
                (incf i)
                ;; Skip the optional 't' after :mmc if present
                (when (and (< i (length args)) (eq (nth i args) t))
                  (incf i)))
               ((eq arg :lmc)
                (setf material-condition :lmc)
                (incf i)
                ;; Skip the optional 't' after :lmc if present
                (when (and (< i (length args)) (eq (nth i args) t))
                  (incf i)))
               ;; Otherwise it's part of the selector spec
               (t
                (push arg selector-spec)
                (incf i))))
    ...
    (unless tolerance-zone
      (error "~A requires :tolerance parameter" gdt-type))
    ...
    (clad.gdt:validate-geometric-tolerance
     ...
     :datum-ref datum-ref
     :tolerance-zone (if (numberp tolerance-zone) tolerance-zone 0.1)  ; Use dummy value if symbolic
     :material-condition material-condition
     :bilateral bilateral)
    ...
    ;; Generate code based on GD&T type
    (let ((constructor-call
           (case gdt-type
             ;; Form tolerances
             ((:flatness :straightness :circularity :cylindricity)
              `(,(intern (format nil "MAKE-~A-TOLERANCE" (symbol-name gdt-type))
                         :clad.gdt)
                ',selector-spec
                ,tolerance-zone))
             ...
             ((:perpendicularity :parallelism)
              (unless datum-ref
                (error "~A requires :datum-ref parameter" gdt-type))
              `(,(intern (format nil "MAKE-~A-TOLERANCE" (symbol-name gdt-type))
                         :clad.gdt)
                ',selector-spec
                ,tolerance-zone
             ...
             (:angularity
              (unless datum-ref
                (error "~A requires :datum-ref parameter" gdt-type))
              (unless angle
                (error "~A requires :angle parameter" gdt-type))
              `(clad.gdt:make-angularity-tolerance
                ',selector-spec
                ,tolerance-zone
                ,datum-ref
                ,angle
             ...
             (:position
              (unless datum-refs
                (error "~A requires :datum-refs parameter" gdt-type))
              `(clad.gdt:make-position-tolerance
                ',selector-spec
                ,tolerance-zone
                ',datum-refs
                :material-condition ,material-condition))
             ...
             ((:profile-surface :profile-line)
              `(,(intern (format nil "MAKE-~A-TOLERANCE" (symbol-name gdt-type))
                ...
                ',selector-spec
                ,tolerance-zone
                ,@(when datum-refs `(:datum-refs ',datum-refs))
                :bilateral ,bilateral))
             ...
             ((:circular-runout :total-runout)
              ...
              `(,(intern (format nil "MAKE-~A-TOLERANCE" (symbol-name gdt-type))
                ...
                ,datum-ref))
             (t
              (error "Unknown GD&T type: ~A" gdt-type)))))

      ;; Generate code to add tolerance to metadata
      `(let* ((current-shape (get-result))
              (core-shape (if (typep current-shape 'clad.shapes:cad-shape)
                              (clad.shapes::core-shape current-shape)
                              current-shape))
              (tolerance ,constructor-call))

         ;; Add geometric tolerance to shape metadata
         (setf (clad.core:shape-metadata core-shape)
               (clad.gdt:add-geometric-tolerance-to-metadata
                (clad.core:shape-metadata core-shape)
                tolerance))

         ;; Return nil (tolerance definition doesn't change the shape)
         nil))))
